...
public class UsuarioMusicaDAO {
            ...
            while (resultado.next()) {
            	...
            	for(int i = 0; i < musicas.size(); i++) {
            		if(musicas.get(i).getId() == idMusica) {
            			usuarioMusica.setMusica(musicas.get(i));
            			break;
            		}
            	}
    ...
    public int apagarUsuarioMusicaId(int idUsuarioMusica) {
        ...
        try { 
            sql = "DELETE FROM usuarioMusica WHERE idUsuarioMusica="+idUsuarioMusica;
            pstmt = conector.getConexao().prepareStatement(sql);  
            resultado = pstmt.executeUpdate(); 
